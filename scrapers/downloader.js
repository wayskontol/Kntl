...
async function ytdl(id) {
  try {
    const response = await axios.get('https://itzpire.com/download/youtube?url=' + id);
    return response.data.data;
  } catch (error) {
    console.error(error);
  }
}
async function fbdl(query) {
 try {
   const id = await fetch('https://anabot.my.id/api/download/facebook?url='+ query +'&apikey=DitzOfc')
...
async function twitterdl(url) {
 try {
  const id = await fetch('https://itzpire.com/download/twitter?url='+ url)
